...
// * This works, but better idea: Start from location 0, see what seed you end up on, check if that is a valid seed
// * My answer was ~5.2 million and took 10 minutes to run (not good!)
...
for (let i = 0; i < seeds.length; i += 2) {
    for (let k = 1; k < seeds[i + 1]; k++) {
        for (let j = 0; j < stsSource.length; j++) {
            if (stsSource[j] <= seeds[i] + k && stsSource[j] + stsRange[j] - 1 >= seeds[i] + k) {
                found = true;
                soil = stsDest[j] - stsSource[j] + seeds[i] + k;
            }
        }
        if (!found) {
            soil = seeds[i];
