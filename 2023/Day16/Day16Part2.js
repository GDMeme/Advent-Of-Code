...
for (let i = 0; i < arr.length; i++) {
    for (let j = 0; j < arr[i].length; j++) {
        ...
        let start;
        if (i === 0) {
            start = { x: j, y: i, direction: 'down' };
        } else if (j === 0) {
            start = { x: j, y: i, direction: 'right' };
        } else if (i === arr.length - 1) {
            start = { x: j, y: i, direction: 'up' };
        } else if (j === arr[i].length - 1) {
            start = { x: j, y: i, direction: 'left' };
        }
        ...
        while (frontier.length !== 0) {
            ...
            if (cell === '.') {
                ...
                const reflect = { right: 'up', up: 'right', left: 'down', down: 'left' };
                const newDirection = reflect[direction];
                ...
                const reflect = { right: 'down', down: 'right', left: 'up', up: 'left' };
                const newDirection = reflect[direction];
                const { newX, newY } = getNextPosition(x, y, newDirection);
                if (!outOfBounds(newX, newY) && !cache.has(`${newX},${newY},${newDirection}`)) {
                    frontier.push({ x: newX, y: newY, direction: newDirection });
