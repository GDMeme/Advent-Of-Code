    ...
    for (let i = minX; i <= maxX; i++) {
        for (let j = minY; j <= maxY; j++) {
            for (let k = minZ; k <= maxZ; k++) {
                let counter = 0;
                const active = activeCubes.has([i, j, k].join()); 
                for (const [directionX, directionY, directionZ] of directionArray) {
                    const newX = i + directionX;
                    const newY = j + directionY;
                    const newZ = k + directionZ;
                    if (activeCubes.has([newX, newY, newZ].join())) {
                        counter++;
                    }
                }
                if (active && (counter === 2 || counter === 3)) {
                    newActiveCubes.add([i, j, k].join());
                }
                if (!active && counter === 3) {
                    newActiveCubes.add([i, j, k].join());
                }
