...
let onlyDirectoriesIndex = 0;
let onlyDirectoriesArray = [[]];
        ...
        if (arr[i][2] === 'c') { // cd
            ...
            if (currentIndex === -1) {  // make sure it's a new ls and not a duplicate
                ...
                if (onlyDirectories === true) {
                    onlyDirectories = false;
                    i = i - onlyDirectoriesCounter;
                    while (arr[i] !== undefined && arr[i][0] !== '$') {
                        if (onlyDirectoriesArray[onlyDirectoriesIndex] === undefined) {
                            onlyDirectoriesArray[onlyDirectoriesIndex] = [currentDirectoryPath];
                        } else {
                            onlyDirectoriesArray[onlyDirectoriesIndex].push(currentDirectoryPath); // this just gives me duplicates, ignore
                        }
                        i++;
                    }
                    onlyDirectoriesIndex++;
                }
            i--;
            }
        }
...
console.log(onlyDirectoriesArray);

// console.log(directorySize);
// console.log(directoryPath);
// console.log('length:', directorySize.length);
// console.log('length:', directoryPath.length);

for (let i = 0; i < onlyDirectoriesArray.length; i++) {
    // TODO: looks like if directory -> directory, there is only one directory in the second level directory
    // somehow calculate the sum for these
}

for (let i = 0; i < directorySize.length; i++) {
    let currentDirectory = directoryPath[i]; // 'a.'
