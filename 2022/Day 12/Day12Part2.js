...
let start = [];
for (let i = 0; i < arr.length; i++) { // find all a's
    for (let j = 0; j < arr[i].length; j++) {
        if (arr[i][j] === 'S') {
            start.push({distance: 0, x: j, y: i});
...
function checkNeighbours(distance, a, b, reached, frontier) {
...
for (let i = 0; i < start.length; i++) {
    let frontier = [];
    let reached = [{x: start[i].x, y: start[i].y}];
    let endCoordinates = {};
    checkNeighbours(start[i].distance, start[i].x, start[i].y, reached, frontier);
    while (frontier.length > 0) {
        currentValue = frontier.shift();
        let end = checkNeighbours(currentValue.distance, currentValue.x, currentValue.y, reached, frontier);
    ...
    if (endCoordinates.distance < minDistance) {
        minDistance = endCoordinates.distance;
    }
}
